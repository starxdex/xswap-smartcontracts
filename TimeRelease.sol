...
contract TimeRelease is Ownable {
    ...
    ReleaseInfo[] public releasePool;
    ...
    function addReleaseInfo(
        ...
        if (!EnumerableSet.contains(_releaser, _releaseTo)) {
            ...
            releasePidMap[address(_releaseTo)] = releasePoolLength() - 1;
            EnumerableSet.add(_releaser, _releaseTo);
        } else {
    ...
    function release(uint256 _pid) public onlyOwner returns (bool) {
        ...
        if (totalRelease <= pool.releaseAmount) {
            return false;
        }
    ...
    function batchRelease() public onlyOwner {
        uint256 length = releasePool.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            release(pid);
        }
    }

    function releasePoolLength() public view returns (uint256) {
        return releasePool.length;
    }
}
