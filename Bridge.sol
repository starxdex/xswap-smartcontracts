...
contract Bridge is Ownable {
    ...
    constructor(
        uint256 _chainId,
        address _transferOperator,
        address _withdrawOperator,
        uint256 startOid,
        address firstToken,
        uint256 firstTokenFixFee,
        uint256 firstTokenTransMinLimit,
        uint256 firstTokenTransMaxLimit
    ) public {
        chainId = _chainId;
    ...
    function addSupportedToken(
        address _token,
        uint256 _fixFee,
        uint256 _transMinLimit,
        uint256 _transMaxLimit
    ) public onlyOwner {
    ...
    function transferIn(
        ...
        require(
            _addressTo == msg.sender,
            "addressTo should be same with msgSender"
        );
        require(supportedTokens[_token], "token not supported");
        require(
            tokenInfo[_token].transMinLimit <= _amount,
            "Trans amount is too small"
        );
        require(
            tokenInfo[_token].transMaxLimit > _amount,
            "Trans amount is too big"
        );
    ...
    function existOrder(uint256 fromChainId, uint256 orderId)
        public
        view
        returns (bool)
    {
    ...
    function transferOut(
        ...
        require(
            !transferOutOrderIds[fromChainId][orderId],
            "Duplicate orderId!!"
        );
