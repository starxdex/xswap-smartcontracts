...
contract OktLinkPool is Ownable {
    ...
    function replaceMultLP(address _multLpToken, address _multLpChef)
        public
        onlyOwner
    {
        require(
        ...
        while (length > 0) {
            ...
            IMasterChefOktLink(multLpChef).emergencyWithdraw(
                poolCorrespond[pid]
            );
        ...
        returns (uint256, uint256)
    {
        ...
        if (user.amount > 0) {
            uint256 TokenPending =
                IMasterChefOktLink(multLpChef).pending(
                    poolCorrespond[_pid],
                    address(this)
                );
            accMultLpPerShare = accMultLpPerShare.add(
                TokenPending.mul(1e12).div(pool.totalAmount)
            );
            uint256 userPending =
                user.amount.mul(accMultLpPerShare).div(1e12).sub(
    ...
    function depositXtAndToken(
        ...
        if (user.amount > 0) {
            ...
            uint256 tokenPending =
                user.amount.mul(pool.accMultLpPerShare).div(1e12).sub(
                    user.multLpRewardDebt
                );
            if (tokenPending > 0) {
        ...
        if (_amount > 0) {
            ...
            if (pool.totalAmount == 0) {
                IMasterChefOktLink(multLpChef).deposit(
                    poolCorrespond[_pid],
                    _amount
                );
                ...
                uint256 beforeToken =
                    IERC20(multLpToken).balanceOf(address(this));
                IMasterChefOktLink(multLpChef).deposit(
                    poolCorrespond[_pid],
                    _amount
                );
    ...
    // Withdraw LP tokens from OktLinkPool.
    ...
    function withdrawXtAndToken(
        ...
        if (_amount > 0) {
            uint256 beforeToken = IERC20(multLpToken).balanceOf(address(this));
            IMasterChefOktLink(multLpChef).withdraw(
                poolCorrespond[_pid],
                _amount
            );
            uint256 afterToken = IERC20(multLpToken).balanceOf(address(this));
    ...
    function emergencyWithdrawXtAndToken(uint256 _pid, address _user) private {
        ...
        uint256 beforeToken = IERC20(multLpToken).balanceOf(address(this));
        IMasterChefOktLink(multLpChef).withdraw(poolCorrespond[_pid], amount);
        uint256 afterToken = IERC20(multLpToken).balanceOf(address(this));
        pool.accMultLpPerShare = pool.accMultLpPerShare.add(
            afterToken.sub(beforeToken).mul(1e12).div(pool.totalAmount)
        );
