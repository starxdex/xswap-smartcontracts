...
contract iXTToken is DelegateERC20, Ownable {
    uint256 private constant maxSupply = 14300000 * 1e18; // the total supply
    ...
    struct MinterInfo {
        address minter;
        uint256 mintSupply;
        uint256 maxMineSupply;
    }

    mapping(address => MinterInfo) public minterInfo;

    constructor(uint256 marketMineSupply) public ERC20("XSwap Token", "iXT") {
        ...
        returns (bool)
    {
        ...
        uint256 _minAmount = maxSupply.sub(totalSupply());
        if (_amount > _minAmount) {
            _amount = _minAmount;
        }
        if (
            minterInfo[msg.sender].mintSupply >=
            minterInfo[msg.sender].maxMineSupply
        ) {
            return false;
        }
        _minAmount = minterInfo[msg.sender].maxMineSupply.sub(
            minterInfo[msg.sender].mintSupply
        );
        if (_amount > _minAmount) {
            _amount = _minAmount;
        ...
        if (_amount > 0) {
            _mint(_to, _amount);
            minterInfo[msg.sender].mintSupply = _amount.add(
                minterInfo[msg.sender].mintSupply
            );
            return true;
        }
    ...
    function addMinter(address _addMinter, uint256 _maxMineSupply)
        public
        onlyOwner
        returns (bool)
    {
        ...
        minterInfo[_addMinter].maxMineSupply = _maxMineSupply;
        minterInfo[_addMinter].mintSupply = minterInfo[_addMinter].mintSupply;
